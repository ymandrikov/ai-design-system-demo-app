...
export const serviceVersions = sqliteTable("service_versions", {
  ...
  scenario: text("scenario", { enum: ["success", "health_check_failure"] }).notNull().default("success"),
}, (table) => [
...
export const deployments = sqliteTable("deployments", {
  ...
  kind: text("kind", { enum: ["deploy", "retry", "rollback"] }).notNull().default("deploy"),
  ...
  check("valid_completion", sql`(${table.result} is null and ${table.completedAt} is null) or (${table.result} is not null and ${table.completedAt} is not null and ${table.completedAt} >= ${table.startedAt})`),
  uniqueIndex("one_active_deployment").on(table.serviceId, table.environment).where(sql`${table.result} is null`),
