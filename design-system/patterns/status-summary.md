...
- Primary fields use a wrapping flex row with `gap-x-8 gap-y-4`; retain the label
  styling and label/value spacing owned by DescriptionItem. Emphasise plain primary
  values with `text-lg font-semibold`. Keep composed badges' own typography.
- Supporting text is `text-sm`; supporting labels and timestamps use
  ...
  Use wrapping metadata rows, `gap-x-4 gap-y-2` for inline context and
  `gap-x-8 gap-y-4` for description lists. Long commits wrap; do not truncate values.
