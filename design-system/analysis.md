# Семантические компоненты — 2026-09-21

Область: весь принадлежащий проекту UI в `app/` и `components/`, включая
незакоммиченные изменения рабочего дерева. Цель — найти общую ответственность,
которую стоит вынести из повторяющейся разметки. Исходный анализ изменял только этот отчёт. SEM-01 и SEM-02 затем согласованы
владельцем и реализованы 2026-09-21; обоснование предложений ниже сохранено как история.
Предыдущие решения и якоря LP/REC/META сохранены ниже как история.

## Приоритетные рекомендации текущего анализа

| Приоритет | Кандидат | Действие | Основание |
| --- | --- | --- | --- |
| Готово | SEM-01 — BackNavigation | Реализован; три потребителя | Три именованные области возврата с одной ссылкой и общей типографикой |
| Готово | SEM-02 — RequestFeedback | Реализован; два потребителя | Полностью одинаковая структура pending/error в диалоге и retry; форму оставить отдельно |

Первый кандидат имеет простую семантическую границу и три применения. Второй убирает
более крупный повтор, но затрагивает объявления вспомогательным технологиям и требует
проверки поведения. Владелец согласовал обе границы запросом «сделай 1 и 2».
Реализации и контракты: [BackNavigation](components/back-navigation.md),
[RequestFeedback](components/request-feedback.md). URL, внешние отступы, Suspense,
условия отображения и состояние запросов сохранены; DeployForm не менялся.
Проверки реализации: pnpm lint, pnpm exec tsc --noEmit, webpack production build,
структурные проверки трёх затронутых контрактов и актуальность индексов прошли.
Статический React-рендер подтвердил idle/pending/error и совместное pending+error:
контейнер polite сохраняется, idle скрыт через sr-only, progress предшествует alert.
Рендер BackNavigation сохранил имя nav, text-s и URL вложенной ссылки.
Обычный аудит выбора: возврат из дочерней записи к списку подходит; inline-ссылка
и переключатель окружения не подходят; Suspense с временно отсутствующей ссылкой
допустим. Для RequestFeedback подходит ошибка повторного запуска, не подходит
валидация radio-поля; пустая ошибка с pending=false оставляет скрытый контейнер.
Все три BackNavigation и оба RequestFeedback сверены с этими границами.

Далее сохранено исходное обоснование и план проверок до реализации.

### SEM-01 — BackNavigation

**Тип:** компонент. **Действие:** extract new. **Статус:** реализован.

Источники и все три прямых потребителя:

- [ServicePage](../app/services/[slug]/page.tsx), строка 44: `nav`, имя Back to services,
  `mb-2xl text-s`, ссылка ← Services с выбранным environment.
- [DeploymentPage](../app/deployments/[id]/page.tsx), строка 44: `nav`, имя Back to service,
  `mb-2xl text-s`, ссылка с именем сервиса и окружением конкретного деплоя.
- [ServiceNotFound](../app/services/[slug]/not-found.tsx), строка 20: та же область,
  но `mt-2xl text-s`, а ссылка получает environment внутри Suspense/BackToServices.

**Общая ответственность:** обозначить отдельную область возврата к родительскому
контексту и обеспечить единое оформление. Это компонент навигации, а не layout
страницы: главным является назначение области, а не расстояние между блоками.

**Минимальная граница:** BackNavigation владеет `nav`, обязательным доступным именем
и `text-s`. Получает имя области и содержимое — одну обычную TextLink, в том числе
через существующий Suspense. TextLink сохраняет владение ссылкой, hover и focus.
URL, видимая подпись со стрелкой, выбор environment и загрузка остаются у потребителя.
Такое выделение сохраняет существующую границу Suspense: nav остаётся снаружи.
Общий компонент не вызывает router.back(): это переход к заданному родителю.

`mb-2xl`/`mt-2xl` оставить на внешней обёртке страницы: у третьего потребителя
навигация стоит после заголовка. Не вводить варианты service/deployment/not-found,
настройки направления отступа или обязательное место над PageHeader.

**Альтернативы:** [TextLink](components/text-link.md) уже подходит для ссылки, но
прямо оставляет landmarks и окружающую композицию потребителям.
[NavigationalTabs](components/navigational-tabs.md) предназначен для группы соседних
назначений. [PageHeader](components/page-header.md) и [PageContent](layouts/page-content.md)
не владеют возвратом. Расширение TextLink до nav изменило бы смысл всех inline-ссылок.
[Deployment not-found](../app/deployments/[id]/not-found.tsx) содержит обычную Services
ссылку без этой области; автоматически добавлять ей nav и стрелку не предлагается.
Cancel формы также остаётся TextLink.

**Польза:** один владелец семантики области и размера текста вместо трёх независимых
реализаций. **Риски:** потерять имя landmark, поменять контекст URL, перенести nav
внутрь Suspense или случайно централизовать внешние отступы. Новый API аддитивный;
существующий TextLink и остальные навигационные потребители менять не требуется.

**Пересмотр LP-03:** прежний вывод рассматривал общий верх страницы и два detail-nav.
Текущий анализ учитывает третий nav в fallback и предлагает отдельную малую границу,
которая сохраняет различное размещение. Отказ от общего AppShell остаётся в силе.

**Следующий шаг:** отдельный craft для этой границы и контракта, затем use в трёх
местах. Закрепить передачу имени и допустимого содержимого; при реализации проверить
длинную подпись, focus, прямое открытие страницы, оба environment и fallback.
Чтение источников подтверждает повторы; визуальная и навигационная эквивалентность
в браузере в этом анализе не проверялась.

### SEM-02 — RequestFeedback для retry и подтверждения

**Тип:** компонент. **Действие:** extract new в узкой области. **Статус:** реализован для retry и диалога; общий компонент всех ошибок не вводился.

Два идентичных фрагмента:
[ConfirmationDialog](../components/ui/confirmation-dialog.tsx), строки 72–79, и
[DeploymentActions](../app/deployments/[id]/deployment-actions.tsx), строки 94–101.
Оба содержат постоянно смонтированный `div aria-live="polite"`, скрытый через sr-only
при отсутствии сообщений, затем pending-параграф и error-параграф с `role="alert"`.
Совпадают порядок, условия и классы; различается только источник pendingLabel.

**Граница:** отображение переданных pending, pendingLabel и error, включая существующую
разметку сообщений. Потребители сохраняют запросы, состояние, очистку ошибок, permissions,
навигацию и место отображения. В DeploymentActions остаётся условие `!rollbackVersion`;
в диалоге обратная связь остаётся внутри popup. ConfirmationDialog сохраняет свою
публичную ответственность за feedback и может делегировать только его рендеринг.

**Альтернативы:** [ConfirmationDialog](components/confirmation-dialog.md) уже закрывает
обратную связь внутри подтверждения, но использовать весь диалог для обычного retry
неправильно. Button не владеет сообщениями. DescriptionItem предназначен для свойств,
DeploymentResult — для завершённого деплоя, а не результата запроса на запуск.

[DeployForm](../app/services/[slug]/deploy/deploy-form.tsx), строки 78–85, пока исключить:
там error идёт первым, есть `aria-atomic="true"`, `id="deploy-error"` и связь radio
через aria-describedby. Объединение всех трёх сейчас потребует дополнительных настроек
и отдельного решения об объявлениях. Состояние загрузки маршрута тоже другая задача.

**Польза:** исключить рассинхронизацию двух одинаковых реализаций feedback.
**Риски:** потерять постоянное присутствие live region, изменить порядок сообщений,
получить другие или повторные объявления. Совпадение JSX не доказывает правильность
сочетания polite и alert. Изменение этой схемы не входит в предлагаемое извлечение.
Публичный API ConfirmationDialog можно сохранить без изменений.

**Уточнение REC-04/LP-04:** прежний отказ от универсального feedback для формы, retry
и диалога сохраняется. Новое предложение ограничено двумя совпадающими поддеревьями;
форма и общие request-state hooks в него не входят. Общий контракт объявлений ещё
нужно подтвердить, поэтому этот кандидат ниже BackNavigation.

**Следующий шаг:** bounded craft с сохранением DOM и consumer-условий, затем проверки
компонента и обеих композиций. Нужны idle/pending/error, повторная попытка, открытие
и закрытие диалога, проверка фактических объявлений. Анализ исходников этого не заменяет.

## Другие проверенные совпадения
...
| TableCaption sr-only и Completed at (UTC) в двух таблицах | Reuse existing Table/Time: названия наборов, колонки и timezone-контекст уже принадлежат потребителям. Компонент одной подписи колонки или общий DataTable не даёт самостоятельной ответственности. |
| Текущая и целевая версии в форме и rollback | Сохранить LP-02 как предложение документированного pattern. VersionLabel/DescriptionItem уже переиспользуются; выбор цели и подтверждение готовой цели различаются. Общий компонент не требуется. |
| Группы controls, submit/Cancel, retry/rollback и modal footer | Keep local: разные порядок, выравнивание и задача; модальный footer уже принадлежит ConfirmationDialog. Общий ActionBar потребует вариантов ради разных обязанностей. |
| Stages, logs, radio-список, production warning | Keep local: по одному месту композиции; map по данным не означает несколько независимых потребителей. Независимая сложная ответственность, требующая выделения сейчас, не установлена. |
| Loading/error/not-found | Reuse PageContainer/PageHeader; сохранить локальные busy/output, retry и переходы. DatasetEmptyState не подходит по контракту. Ранее отмеченная возможность перенести Try again в PageHeader.controls остаётся отдельной use-задачей, не новым компонентом. |

Эти решения сохраняют LP-01, REC-01–03 и META-01 как реализованные. Отказы от
универсальных Card, Label, SummaryGrid и ActionBar сохраняются. Новых кандидатов
на adoption локального визуального компонента не найдено; RefreshActiveDeployment
отвечает за обновление данных и не требует UI-контракта ради этого анализа.

## Покрытие и доказательства текущего анализа

| Область | Результат |
| --- | --- |
| Четыре основные страницы | Прочитаны app/page.tsx, app/services/[slug]/page.tsx, app/services/[slug]/deploy/page.tsx, app/deployments/[id]/page.tsx |
| Локальный UI | Прочитаны deploy/deploy-form.tsx и deployments/[id]/deployment-actions.tsx, включая условия, handlers и связи сообщений с полями |
| Четыре fallback views | Прочитаны services/[slug]/not-found.tsx и deployments/[id]/{loading,error,not-found}.tsx |
| Общий UI | Прочитаны все components/ui/*.tsx, tabs-styles.ts, components/layouts/*.tsx и components/deployments/*.tsx |
| Стили и оболочка | Прочитаны app/layout.tsx, app/globals.css, design-system/tokens/{README.md,raw.css,semantic.css} |
| Система и зависимости | Проверены три индекса, статусы 24 контрактов, относящиеся к кандидатам публичные границы, DESIGN.md, прежний analysis.md и gaps.md; прочитаны lib/deployments/presentation.ts и with-design-system-exception.tsx |

Все области выбранного UI учтены; pending/unavailable областей нет. Все 24 контракта
имеют discoverable-статус; скрытой или deprecated-альтернативы предложенным границам
не обнаружено. Источники и количества перепроверены поиском по всем app/components.
Отдельных примеров и тестов в этих каталогах не найдено; примерами служат потребители
и примеры в контрактах. Серверные actions, БД и simulation не аудировались на корректность
логики; зависимости и generated-копии исключены.

Это анализ извлечения по исходникам, без изменения runtime-кода и без утверждений о
визуальной эквивалентности или улучшении доступности. Build, lint и браузерные сценарии
не запускались: выводы о повторах от них не зависят. Проверены ссылки отчёта и доступность
контрактов из индексов. Новые gap-записи не добавлены: эти возможности извлечения сами
по себе не свидетельствуют о системном нарушении. Предложения требуют отдельного
craft → checks → use → checks; анализ не запускает реализацию.

## История до текущего анализа

Ниже сохранены предыдущие отчёты. Их количества, классы, результаты проверок и планы
относятся к соответствующим датам; текущие выводы приведены выше.
